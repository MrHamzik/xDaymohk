 ...
 *
 * Звуки длятся до ~0.75 с и негромкие: уведомление не должно пугать
 * человека в мечети или на встрече. Первая версия была вдвое короче —
 * сигнал успевал оборваться раньше, чем его замечали, и все мелодии
 * звучали одинаковым щелчком. Громкость по умолчанию 0.3 от максимума.
 */
...
const SCORES: Record<Exclude<SoundId, 'none'>, Tone[]> = {
  // Мягкий колокольчик: две ноты вверх с долгим послезвучием.
  chime: [
    { freq: 880, at: 0, dur: 0.38 },
    { freq: 1318.5, at: 0.18, dur: 0.55 },
  ],
  // Одиночный сигнал — нейтральный, для частых событий.
  ding: [
    { freq: 1046.5, at: 0, dur: 0.36 },
    { freq: 1046.5, at: 0.02, dur: 0.34, gain: 0.35, type: 'triangle' },
  ],
  // Двойной клик: заметнее одиночного, но не тревожный.
  double: [
    { freq: 987.8, at: 0, dur: 0.22 },
    { freq: 987.8, at: 0.26, dur: 0.3 },
  ],
  // Тихий низкий тон: для фоновых событий, почти незаметен.
  soft: [
    { freq: 523.3, at: 0, dur: 0.5, gain: 0.6, type: 'triangle' },
    { freq: 784, at: 0.1, dur: 0.36, gain: 0.28, type: 'triangle' },
  ],
  // Тревожный: две ноты ВНИЗ, так слух читает «внимание».
  alert: [
    { freq: 880, at: 0, dur: 0.28, type: 'square', gain: 0.5 },
    { freq: 587.3, at: 0.28, dur: 0.44, type: 'square', gain: 0.5 },
  ],
  // «Капля»: быстрый удар и долгий мягкий хвост.
  drop: [
    { freq: 1318.5, at: 0, dur: 0.16 },
    { freq: 659.3, at: 0.12, dur: 0.5, type: 'triangle' },
  ],
};
