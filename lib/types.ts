...
/**
 * Через сколько суток после истечения срока просроченное задание
 * УДАЛЯЕТСЯ из базы физически.
 *
 * Просроченное — это задание, которое никто не взял: исполнителей у
 * него нет, отзывов тоже (они ставятся только по завершённой сделке),
 * поэтому удалять безопасно — история ни на чём не держится.
 *
 * Раньше такие записи просто помечались is_archived и лежали в базе
 * вечно, накапливаясь без всякой пользы. Неделя отсрочки нужна, чтобы
 * заказчик успел увидеть уведомление «срок истёк» и при желании
 * пересоздать задание, пока помнит его содержание.
 */
export const TASK_EXPIRED_DELETE_DAYS = 7;
