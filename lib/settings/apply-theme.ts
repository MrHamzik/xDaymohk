...
export function applyThemeColors(
  ...
  // Градиент шапки — РОВНО между двумя настроенными цветами.
  //
  // Прежняя формула брала не те цвета и гасила края: слот «Шапка
  // каталога (начало)» вообще не использовался, вместо него шёл главный
  // цвет, затемнённый на 55 %, а конец дополнительно затемнялся на
  // 20 %. Получалось четыре точки, где обе крайние темнее середины, —
  // отсюда «уходит в тёмное по краям». Пользователь при этом правил
  // heroFrom и не видел эффекта.
  //
  // Теперь две точки: heroFrom → heroTo. Промежуточная на 50 % не
  // нужна — она бы снова добавила перелом яркости.
  set(
    '--smk-hero-gradient',
    `linear-gradient(135deg, ${colors.heroFrom} 0%, ${colors.heroTo} 100%)`,
  );
