...
export interface UserSettings {
  ...
  welcomeSent: boolean;
  /**
   * Автосохранение места чтения (п.9 ТЗ Этапа 2).
   *
   * Включено — позиция остановки пишется в базу в реальном времени при
   * обычном чтении. Не действует в режиме исследования и при переходах
   * из поиска (п.7): там сохранённая точка остаётся нетронутой.
   * Синхронизировано с чекбоксом одноразовой подсказки (п.8).
   */
  readingAutosave: boolean;
  /**
   * Одноразовая подсказка о сохранении прогресса уже показана (п.8 ТЗ
   * Этапа 2). Флаг на аккаунте (БД), а не в браузере: подсказка
   * показывается один раз за всё время аккаунта, на любом устройстве.
   */
  readingTipShown: boolean;
