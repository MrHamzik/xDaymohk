...
export interface ThemeColors {
   ...
   *
   * Пять слотов, а не один: у сообщения есть СМЫСЛ, и он должен
   * читаться до чтения текста. Общий фон один на все четыре типа —
   * тогда блоки выглядят одной семьёй, а различает их цвет текста
   * и полоса-засечка. Так подсказка не превращается в четыре разных
   * по яркости пятна на одной карточке.
   *
   ...
   */
  /** Общий фон всех блоков-подсказок. */
  noteBg: string;
...
export const THEME_COLOR_GROUPS: Record<ThemeColorGroup, Array<keyof ThemeColors>> = {
  ...
  specific: [
    ...
    'noteBg', 'noteInfo', 'noteWarn', 'noteDanger', 'noteSuccess',
  ],
};
