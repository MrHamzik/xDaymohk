...
export const DEFAULT_QUICK_WIDGETS = ['status', 'lang', 'notify', 'theme'];
...
export const DEFAULT_SETTINGS: UserSettings = {
  ...
  // Четыре быстрые настройки (п.12). «Уведомления», «Язык», «Режим
  // работы» и «Тема оформления» есть в разделе «Дополнительно», но
...
> = {
   ...
   * лимонно-оливковый 80° (статус «работает»); акцент — золото 40°.
   *
   ...
   *
   * ХОЛОДНЫХ ЦВЕТОВ В ЭТОЙ ТЕМЕ НЕТ. Раньше акцент был бирюзовым
   * (#1face0), «свободный график» — голубым (#3d9ec2), а подсказка
   * «информация» строилась из этого же голубого. На кремовом полотне
   * из тёплой охры и золота они читались как чужие наклейки: тема
   * заявлена янтарной, а половина активных элементов — морская.
   *
   * Теперь весь круг лежит в тёплом секторе 338°…80°:
   *   золото 40°  — акцент, ссылки, «золотые» кнопки;
   *   охра 28°    — интерфейс, кластеры карты, метка «проверен»;
   *   янтарь 42°  — «перерыв», дома на карте;
   *   олива 80°   — «работает»;
   *   терракота 12° — опасность;
   *   бордо 338°  — «свободный график» и подсказка-информация.
   *
   * Бордо взят вместо голубого как самый «холодный из тёплых»: он на
   * 34° отстоит от терракоты (порог различимости подсказок — 25°),
   * поэтому «информация» и «опасность» не сливаются, и при этом
   * остаётся в красно-жёлтой семье.
   */
  amber: {
    name: 'Янтарь',
    isDark: false,
    colors: {
      ...
      statusActive: '#7c9f38',
      statusBreak: '#ecab13',
      // Бордо вместо голубого: единственный «прохладный» тон темы,
      // но всё ещё из красно-жёлтой семьи. Контраст к карточке 5.86.
      statusFlexible: '#ae3763',
      ...
      // Подсказки: общий фон и четыре смысловых цвета текста
      // (контраст к фону не ниже 4.5 — текст мелкий).
      // «Информация» теперь бордовая 338°, а не голубая: выводится из
      // statusFlexible, как и во всех остальных темах.
      noteInfo: '#711e3c',
      noteInfoBg: '#f7bfd4',
      noteWarn: '#71581e',
      noteWarnBg: '#f7e6bf',
...
export function settingsFromDb(row: Record<string, unknown> | null): UserSettings {
  ...
  return normalizeSettings({
    ...
    compactLists: row.compact_lists,
    confirmDanger: row.confirm_danger,
    quietHours: row.quiet_hours,
    vibrate: row.vibrate,
    hidePrayer: row.hide_prayer,
    lightMode: row.light_mode,
...
export function settingsToDb(settings: UserSettings): Record<string, unknown> {
  return {
    ...
    compact_lists: settings.compactLists,
    confirm_danger: settings.confirmDanger,
    quiet_hours: settings.quietHours,
    vibrate: settings.vibrate,
    hide_prayer: settings.hidePrayer,
