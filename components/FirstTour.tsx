...
export default function FirstTour({ onDone, onCardVisible }: FirstTourProps) {
  ...
  const goNext = () => {
    ...
    if (step.awaits && !tasking) {
      setTasking(true);
      // Меню «+» гид больше НЕ открывает за человека (п.3).
      //
      // Раньше «Дальше» само слало plus-open: шаг просит «нажмите
      // плюс», а меню разворачивалось само по себе — нажимать было уже
      // нечего, и смысл шага пропадал. Остальные шаги-задания
      // (каталог, меню) всегда ждали действия руками; этот выбивался.
      //
      // Теперь одинаково везде: карточка уходит, подсветка остаётся на
      // кнопке, ждём настоящего нажатия.
      return;
    }
    if (last) finish();
