  ...
  return (
    <article
      ...
      className={`smk-lux smk-rays smk-enter group flex h-full cursor-pointer flex-col overflow-hidden text-slate-900 dark:text-white ${
        ...
        // Отменённые удаляются сразу (обновление 42). Метка остаётся
        // подстраховкой: если удаление не прошло, карточка не должна
        // выглядеть живой.
    ...
    >
      ...
      <div className="mt-auto flex flex-wrap items-center gap-1.5 px-4 py-3 smk-text-label font-bold">
        ...
        {task.status === 'cancelled' && (
          <span className="inline-flex items-center gap-1 rounded-lg bg-rose-50 px-2 py-1 text-rose-700 dark:bg-rose-950/50 dark:text-rose-300">
        ...
        {isDisputed ? (
          /* Спор занимает место метки срока: у него нет обратного
             отсчёта, важно состояние. Оформление — как у всех
             остальных меток, отличаются только иконка, текст и цвет. */
          <span className="inline-flex items-center gap-1 rounded-lg bg-rose-50 px-2 py-1 text-rose-700 dark:bg-rose-950/50 dark:text-rose-300">
