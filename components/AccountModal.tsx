    ...
    <>
      ...
      <div className="flex max-h-[90vh] w-full flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl dark:bg-zinc-800 sm:max-w-md sm:rounded-2xl">
        ...
        {isLoading ? (
          ...
          <form onSubmit={handleSaveAccount} className="space-y-3.5 overflow-y-auto p-4">
            {account.isBlocked && <p className="rounded-xl border border-red-200 bg-red-50 p-2.5 text-xs font-semibold text-red-700 dark:border-red-200 dark:bg-red-50 dark:text-red-700">Ваш аккаунт заблокирован администратором. Новые анкеты создавать нельзя.</p>}
            ...
            <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs text-slate-900 focus:outline-none focus:ring-1 focus:ring-emerald-500 dark:border-zinc-800/60 dark:bg-zinc-800 dark:text-white"
                />
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
              ...
              <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-3 dark:border-zinc-800/60 dark:bg-zinc-900">
                <label htmlFor="account-gender" className="mb-1 block text-xs font-bold text-slate-700 dark:text-zinc-400">Пол</label>
                <select id="account-gender" value={gender} onChange={(event) => setGender(event.target.value as any)} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 pr-10 py-2.5 text-xs text-slate-900 focus:outline-none focus:ring-1 focus:ring-emerald-500 dark:border-zinc-800/60 dark:bg-zinc-800 dark:text-white">
              ...
              <div className="rounded-xl border border-slate-100 bg-slate-50/60 p-3 dark:border-zinc-800/60 dark:bg-zinc-900">
                <label htmlFor="account-birthDate" className="mb-1 block text-xs font-bold text-slate-700 dark:text-zinc-400">Дата рождения</label>
                <input id="account-birthDate" type="date" value={birthDate} onChange={(event) => setBirthDate(event.target.value)} className="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-xs text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-emerald-500 dark:border-zinc-800/60 dark:bg-zinc-800 dark:text-white" />
              </div>
            </div>

            <section className="space-y-2 rounded-xl border border-slate-100 bg-slate-50 p-3 dark:border-zinc-800/60 dark:bg-zinc-900">
                ...
                <div key={profile.id} className={`flex items-center gap-2 rounded-xl border p-2 ${profile.isPersonal ? 'border-emerald-200 bg-emerald-50/60 dark:border-emerald-900 dark:bg-emerald-950/20' : profile.isHidden || profile.isBanned ? 'border-red-300 bg-red-50 dark:border-red-900 dark:bg-red-950/30' : 'border-slate-200/60 bg-white dark:border-zinc-800/60 dark:bg-zinc-900'}`}>
                  <div className="min-w-0 flex-1">
                    <div className="flex min-w-0 items-center gap-1.5">
                      ...
                      {profile.isPersonal && <span className="shrink-0 rounded bg-emerald-100 px-1 py-0.5 text-[8px] font-bold text-emerald-800">Личная</span>}
                    ...
                    <p className="truncate text-[10px] text-slate-500 dark:text-zinc-500">{profile.isPersonal ? 'Без контактов, всегда видна' : profile.workplaceAddress}</p>
                  </div>
