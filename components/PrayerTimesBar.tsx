        ...
        <div className="grid grid-cols-5 gap-1 smk-text-label sm:smk-text-label font-semibold text-center">
          {prayersOnly.map((prayer) => {
            ...
            return (
              <div
                key={prayer.id}
                // Неактивные намазы (п.5, повтор 22.08): в светлых
                // темах --smk-card-inset почти сливался с подложкой
                // виджета — плитки не читались. В свете берём явный
                // контрастный slate, в тёмных темах — штатный inset,
                // который там на шаг светлее панели.
                className={`flex flex-col items-center justify-center rounded-lg py-1 px-0.5 transition ${
                  isActive
                    ? 'bg-emerald-600 text-white font-black shadow-sm'
                    : 'bg-slate-200/80 text-slate-600 dark:bg-[var(--smk-card-inset)] dark:text-[var(--smk-muted-bright)]'
                }`}
              >
